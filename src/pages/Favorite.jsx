...
const BASEURL = "http://127.0.0.1:8000/favors";
    ...
    <div className="flex flex-col gap-3 justify-between  h-screen" style={{ height: "calc(100vh - 9rem)" }}>
      <div>
        <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {Array.isArray(data) && data.map((stock) => <StockCard key={stock.stockinfo.symbol} currentStock={stock.stockinfo} />)}
        </ul>
      </div>
      <Pagination name={"favorite"} className="mb-12" totalPages={pageNum} currentPage={1} />
    </div>
  );
}
